...
# Creates a new array without cooling load data. (Our outcome)
X = np.array(data.drop([predict_heating, predict_cooling], 1))  # Attributes w/o Labels.
Y = np.array(data[predict_heating])  # Labels only. (The outcomes derived from attributes)
...
for _ in range(30):
    ...
    if acc > best:
        best = acc
        with open("EnergyEfficiency-Heating.pickle", "wb") as f:
            pickle.dump(linear, f)
# Block of code that user comments out after model achieves desired accuracy.

pickle_in = open("EnergyEfficiency-Heating.pickle", "rb")
...
p = "Surface Area"
style.use("ggplot")
pyplot.scatter(data[p], data["Heating Load"])
pyplot.xlabel(p)
pyplot.ylabel("Final Heating Load")
pyplot.show()
